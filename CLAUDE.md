...
- **localStorage is used only for:**
  ...
  - `sb:render-anchors:{matchId}` — per-device video↔event sync anchors from `/m/[id]/render`, keyed inside the value to the uploaded file's name+size so a different recording starts clean instead of inheriting a stale mapping. Restored automatically when the same file is re-picked; device-local because the video file itself is.
