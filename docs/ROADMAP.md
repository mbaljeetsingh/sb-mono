...
| **~~E1.21 · Tennis + pickleball + table-tennis presets~~** ✅ done 2026-09-07 | All four sports enabled on `/new`, plus padel and squash (six total), table-tennis doubles included. Turned out not to be "just config files": official pickleball needed side-out scoring and official tennis/padel needed a point tier below the game, so `RacquetConfig.scoring` now selects `rally` / `side-out` / `tennis`. Each sport ships its governing body's ruleset plus a simplified variant where one is commonly played. Courts carry their real proportions and line geometry per sport. | done |
