...
| **E1.11 · Offline-first event sync (`useIDBKeyval` + Supabase Realtime)** | Replace localStorage in `useEvents` with **`useIDBKeyval`** (VueUse wrapper around `idb-keyval`) as durable offline queue + Supabase as source-of-truth + Realtime INSERT/DELETE subscription for cross-device sync. Drop-in for the current `useStorage('sb:events:{matchId}', [])` pattern (same ref-based API, just backed by IndexedDB). Sync loop: write event → IDB → BroadcastChannel (cross-tab) → attempt Supabase insert; on failure mark `synced=false`, retry on `online` event + every ~10s. Add "Offline — N events queued" pill in `AppHeader` when backlog exists. Promoted from "future-proofing" to **v1 requirement** because venue WiFi flakes and 5G isn't reliable mid-match — points must not be lost. Keep BroadcastChannel for instant cross-tab paint. SQLite via `@capacitor-community/sqlite` stays deferred to E2.9 behind a thin `Storage` interface. ARCHITECTURE §4.5 to be updated when shipped. | 1 weekend |
