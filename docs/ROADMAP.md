...
| **E1.14 · Privacy policy + ToS + DPDP/GDPR** | Deferred to marketing site — legal pages live in `np-mono/apps/web/public/` (served at `/privacy` and `/terms` on the website). Sb-mono only needs to link out to those URLs from its footer / auth screens once the marketing site is live. Cookie-free analytics confirmation still applies here (PostHog config). PRD §3.19. | footer links only |
