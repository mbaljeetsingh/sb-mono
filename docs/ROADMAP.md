...
| **E1.28 · Match + per-game timer (broadcast convention)** | Total match elapsed and per-game durations rendered like real BWF/ATP/ITTF broadcasts. New `useMatchTiming(events)` composable derives timing from existing event timestamps — anchors on `match.start`, boundaries on `game.end`, subtracts `suspension.start`/`suspension.end` windows (rain/power), keeps `timeout.*` windows in (TV convention). Exposes `matchElapsedMs` (live), `gameDurationsMs[]`, `currentGameElapsedMs`, `isSuspended`. `formatDuration()` helper in `@sb/shared` (MM:SS under 1hr, H:MM:SS over). Wire-up: (1) `MatchOverModal` shows total + per-game block, (2) saved-result page same, snapshot `matchDurationMs` + `gameDurationsMs[]` into `sb:result:{matchId}` so saved view is self-contained, (3) optional `<MatchClock />` slot themes opt into for live overlays — broadcast-classic top-corner first. Engine stays pure; live ticking via `useNow({ interval: 1000 })`, frozen when `matchEndTs` set or `isSuspended`. Unit tests for suspension-subtraction math in the composable. | 1-2 days |
| **E1.29 · Match settings sheet on /m/[id]** | The Settings icon on `/m/[id]` is currently a disabled placeholder. Open a sheet that writes directly to `sb:meta:{matchId}` for: team A/B names, doubles per-player names (a1/a2/b1/b2), and tournament meta (event name, round, category, court label). All target fields already exist in `MatchMeta` and several themes render them — this only opens the write path. Optional sponsor-name slot ties in with E2.5 once Storage upload lands. Danger-zone section: "Reset match" + "Delete match" (the latter clears localStorage + Supabase rows). Distinct from E1.17 (which is the same data captured on `/new` at creation time). | 1 day |
